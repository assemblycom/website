...
export const getFirmBranding = cache(
  async (ref: string | undefined): Promise<BuiltOnFirm | undefined> => {
    ...
    const firmLogo = await firstLogo([
 ...
 * holds the page.
 */
async function firstLogo(
  candidates: (string | undefined)[],
): Promise<FirmLogo | undefined> {
  const urls = [...new Set(candidates)].filter((url) => url !== undefined);
  const pending = urls.map(measureLogo);
...
async function measureLogo(url: string): Promise<FirmLogo | undefined> {
