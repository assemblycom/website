 ...
 *
 * How the image sits in the square is decided by its proportions, the one
 * property of an upload a browser can actually read — the pixels are on S3
 * without CORS, so a canvas that touched them would be tainted. A near-square
 * upload is a mark or a photograph and fills the tile, since cropping a few
 * percent off its edges costs nothing. Anything longer is a wordmark, where a
 * crop would eat the name, so it stays contained with room around it.
 */
const COVER_MIN_RATIO = 0.75;
const COVER_MAX_RATIO = 1.5;

/**
 * Past this, drop the logo and show the initial instead.
 *
 * The tile leaves 40px of width, so a 3:1 mark renders 13px tall and a 17:1
...
type Fit = "contain" | "cover" | "initial";

/** How an upload of these proportions should sit in the square. */
